...
| 機能 | 詳細 |
| :--- | :--- |
| 🎥 **リアルタイム映像プレビュー** | カメラからの RTP ストリーム（H.264 / MJPEG）をブラウザ上で低遅延表示。全画面・静止画キャプチャ対応。 |
| 💓 **省電力ハートビート** | プレビュー非表示時はバックエンドのストリームデコードを自動停止し、CPU負荷を最小化。 |
| 🌐 **マルチNIC自動検出** | 有線LANとWi-Fiが混在する現場PCでも、カメラ接続用のLANアダプターを選択するだけでIP・ブロードキャストIPを自動補完。 |
